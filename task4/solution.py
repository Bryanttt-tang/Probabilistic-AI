...
def mlp(sizes, activation, output_activation=nn.Identity):
    ...
    for i in range(len(sizes) - 1):
        ...
        layers += [nn.Linear(sizes[i], sizes[i + 1]), act_func()]
...
class Actor(nn.Module):
    ...
    def forward(self, obs, act=None):
        ...
        log_prob = None
        if act is not None:
            log_prob = self._log_prob_from_distribution(pi_dist, act)
        return pi_dist, log_prob
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        deltas = rews[:-1] + self.gamma * vals[1:] - vals[:-1] # 模拟t+1 and t
...
class Agent:
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            pi_dist, log_prob_pi = self.actor.forward(state)
            action = pi_dist.sample()  # sample action
            log_prob_action = pi_dist.log_prob(action)  # get log prob
            value_function = self.critic.forward(state)  # get value
...
def train(env, seed=1):
    ...
    for epoch in range(epochs):
        ...
        pi_distribution, logp = agent.actor.forward(obs, act)

        # there are two kind of policy loss, experiments show TD learning is indeed more stable as the lecture describe
        # " - ": gradient ascending -> gradient descend
        actor_loss = -(logp * tdres).mean()
        # actor_loss = -(logp * ret).mean()  # not stable
        ...
        for _ in range(100):
            ...
            critic_loss = torch.sum((agent.critic.forward(obs) - ret) ** 2)
