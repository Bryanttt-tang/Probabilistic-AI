...
class Model(object):
    ...
    def make_predictions(self, test_features: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        predictions = gp_mean+0.5*gp_std
    ...
    def fitting_model(self, train_GT: np.ndarray, train_features: np.ndarray):
        ...
        # sub_data = self.rng.choice(np.arange(self.N - 1), size=self.num_sample, replace=False)

        # self.train_X = train_features[sub_data]
        # self.train_Y = train_GT[sub_data]

        kmCluster = KMeans(n_clusters=self.num_sample)
        kmCluster.fit(self.X)
        closest, _ = pairwise_distances_argmin_min(kmCluster.cluster_centers_, self.X)
        self.train_X = train_features[closest]
        self.train_Y = train_GT[closest]


        kernel1 = 1.0 * RBF(length_scale=0.025) + 0.2 * RBF(length_scale=5)
        kernel2 = self.output_scale * RBF(length_scale=self.length_scale) + ExpSineSquared(length_scale=1, periodicity=5.0)
        kernel3 = 1.0 * Matern(length_scale=1.0, nu=1.5) + 0.2 * RBF(length_scale=5.0) 
        kernel4 = self.output_scale * RBF(length_scale=self.length_scale) + ConstantKernel(constant_value=2.0)
        self.gaussian_process = GaussianProcessRegressor(kernel=kernel1, alpha=1e-2, n_restarts_optimizer=10)
        self.gaussian_process.fit(self.train_X, self.train_Y)
